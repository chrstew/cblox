...
// sfx
// todo: preload these, and consolidate into a modular PLAY_SOUND function
...
function menuResize () {
	...
	if (menuBlockSize > 75) menuBlockSize = 75;
...
function getSVG8by8(blockType, where) {
	...
	//if (blockType == 'blank') return SVGString;
...
function updateBlock(x, y, blockType, possessionDisplayName, moveNum, origin, color, duration, history, originColor, possession, possessionColorSpread) {
	...
	if (blockType == "blank") {
		...
		if (blockType == 'ice') $(id).addClass('ice');
	...
	*/
	...
	if (typeof color !== 'undefined') {
		...
		if ($('#color-'+ color.substr(1)).length < 1) addNewStyle(color);
...
function get_pos(x,y,cols) { return (y - 1) * cols + x - 1; }
...
function joinGame(gameID, moveCount, timeLeft, players, rows, cols, board, gameType) {
	...
	function youArePlaying() {
		for (var id in players) { if (id == you) return true; }
		return false;
	}
		...
		$("#menuContainer").on({
			mouseenter: function () {
				...
				if ($(this).hasClass('disabled') || (menuState == false)) {
					...
					if (audioEnabled) hoverAudio.play(); // play audio
				}
			},
			mouseleave: function () {
		...
		//$('.menu_block:not(.nohover)').on("click",function() {
			if (menuState == true && standby == false) { // if menu and game are both active
				if ($(this).hasClass('disabled')) {
					...
					socket.emit('GAME_MOVE_ATTEMPT', x, y, type, global_moveCount); // moveCount to check if it was placed at the last split second before the turn: returns invalid move.
				}
			}
		});
...
function collisionUpdate(collisionMode) {
	if (typeof collisionMode !== 'undefined') {
		if (collisionMode.permanence === true) {
			...
			var sString = 's';
			if (collisionMode.permanence === 1) sString = '';
...
function cleanup(winners, reason) {
	...
	if (winners.length > 1) {
		if (audioEnabled) drawgameAudio.play();
	} else {
		if (winners[0] == you) {
			if (audioEnabled) youwinAudio.play();
		} else {
...
function renderExitButton() {
	if ($("#exit").length === 0) {
		...
		$("#exit").on('click', function() { exitGame(); });
	}
}
...
function exitGame() {
	...
	inGame = false;
	showWinState = false;
	socket.emit('GAME_EXIT_TO_LOBBY');
}
...
function renderRoomTitle(title) {
	...
	var width = $("#titleWidth").width();
	$("#titleWidth").remove();
	var maxwidth = 211; // sidebar width minus margin... minus one.
...
function renderGames(lobbyData) {
	...
	for (var i=0; i<lobbyData.length; i++) {
		...
		if (lobbyData[i].creatorElo > 0) appendString += '&ensp;('+ lobbyData[i].creatorElo +')';
		...
		if (lobbyData[i].gameType === 'practice') {
			...
			if (lobbyData[i].opponentElo > 0) appendString += '&ensp;('+ lobbyData[i].opponentElo +')';
		}
		
		appendString += '</span>';

		if (lobbyData[i].gameType === 'random') appendString += '<span class="gameSettings">Random</span>';		
	...
	$(".spectateGameButton").on('click', function() {
		var tempID = $(this).parent().parent().data("gameid");
		socket.emit('GAME_JOIN', tempID, 'spec');
	});
	...
	if (ghostFlow) {
		setTimeout(function(){ 
			...
			if (enticingGame !== 'none') {
				if (debounce = temp) socket.emit('GAME_JOIN', lobbyData[enticingGame].id, 'spec');
			}
...
function updateTimer(timerValue, turnCount) {
	...
	gameTimer = setInterval( function() {
		...
		if (timerValue == 0) {
			...
			if (timerValue == 10) { $("#container").addClass("timeWarning"); }
...
function onlinePlay() {  // hooray! hooray! for online play!
	...
	/*
	//join a game as spectator if gameID is given in url, ex: /game/:gameid
	...
	*/
	...
	socket.on('disconnect', function() {
		...
		if (audioEnabled) disconnectedAudio.play();
	...
	*/
	...
	socket.on('UI_LOBBY_UPDATE', function(lobbyData, leaderData) {
		$("#open").empty();
		$("#inprogress").empty();
		$("#eloRank").empty();
	...
	socket.on('UI_LOBBY_RENDER', function(lobbyData, leaderData, lobbyUserData) {
		player = false;
		$("#lobby").remove();
		$("#leaderboard").remove();
		...
		appendString += '<a id="howToPlay" class="buttonStyle" href="https://docs.google.com/document/d/1c_rIYxdl2udNHXPFnHj5ELoYPjtej4hDjy7nCHnceG0/edit" target="_blank">How to Play</a>';
		appendString += '<a class="buttonStyle" href="https://docs.google.com/document/d/1cIwGEWhQYZUPRn1sBzatFuVSNau3VPHxOu9ffszejU4/edit" target="_blank">Documentation</a>';
		appendString += '<div id="toggleAudio" class="buttonStyle">'+ audioButtonSVG() +'</div>';
		appendString += '</div>';
		...
		$('#newgame').on("click",function() { socket.emit('GAME_NEW'); });
		
		// send the server new game [random board] command
		$('#randgame').on("click",function() { socket.emit('GAME_NEW', 'random'); });
		
		$('#practice').on("click",function() { socket.emit('GAME_PRACTICE_NEW'); });	
		...
		if( !isGhost ){
			if (lobbyUserData.gamesPlayed > 0) {
				var appendString = '';				
				appendString += '<b><span style="color:'+ lobbyUserData.color +'">'+ lobbyUserData.username +'</span></b>';
			...
			if (lobbyUserData.remainingRerolls > 0) {
				...
				$("#rerollColor").on('click', function() { socket.emit('CHAT_COLOR_REROLL'); });
			}
		}
	...
	*/
	...
	socket.on('UI_NO_MORE_COLOR_REROLLS', function (color) { $("#rerollColor").remove(); });
	
	socket.on('update lobby name color', function(color) { $(".lobbyName").css('color', color); });
	
	socket.on('remove player heading', function() { $("#playerRight").empty(); });
	
	socket.on('GAME_SETUP', function(gameID, joinStatus, title, rows, cols, board, players, gameState, blockList, timeLimit, collisionMode, moveCount, timerValue, gameType) {
		// this runs when:
		// - a game is started
				...
				*/
				...
				function creatorSettings() {
					...
					if (joinStatus == 'creator') {
						...
						$("#timeLimit").on('click', function() { socket.emit('time limit setting', 'infin'); });
						...
						$('#moreTime').on('click', function() { socket.emit('time limit setting', 'more'); });
						$('#lessTime').on('click', function() { socket.emit('time limit setting', 'less'); });
						...
						$("#classic").on('click', function() { socket.emit('classic mode'); });
						
						// ADVANCED
						//$("#sidebar").append('<div class="sideButton" id="circlemode">Advanced</div>');
						$("#circlemode").on('click', function() { socket.emit('advanced mode'); });
						
						// RANDOMIZE
						//$("#sidebar").append('<div class="sideButton" id="randomize">Randomize</div>');
						$("#randomize").on('click', function (){ socket.emit('randomize'); });
						...
						$("#blockListEditorBtn").on('click', function() {
							...
							*/
							...
							$(".blockInfoContainer > span:nth-of-type(1)").on('click', function(evt) {
								// subtracting
								var value = $(this).siblings("span:nth-of-type(2)").html();
								if (parseInt(value) !== 0) {
	...
	socket.on('time limit update', function(timeLimit) { timeLimitUpdate(timeLimit); });
	
	socket.on('collision update', function(collisionMode) { collisionUpdate(collisionMode); });
	...
	socket.on('rebuild board', function(rows, cols) { rebuildBoard(rows, cols); });
	...
	socket.on('GAME_KILL', function() { exitGame(); });

	socket.on('build menu', function(blockList) { buildMenu(blockList); });
	...
	*/
	...
	socket.on('all players ready', function(gameID, timeLimit, players, rows, cols, board, gameType) {
		...
		if (audioEnabled) newgameAudio.play();
		
		joinGame(gameID, global_moveCount, timeLimit, players, rows, cols, board, gameType);
	});
	
	socket.on('GAME_MOVE_NEW', function(board, noMove, blockList) {
		global_moveCount++;
		$(".waitMsg").parent().remove();
	...
	socket.on('GAME_VICTORY', function(winners, winPaths, color, gameType) {
		//console.log("VICTORY Recieved");
		//console.log('showWinState', showWinState);
	...
	socket.on('remove rematch button', function(){
		...
		$("#rematch").remove();
		if (ghostFlow) {
			setTimeout(function(){ exitGame(); }, 3000);
		}
	});
	
	socket.on('UI_BOARD_RENDER', function(data) {
		renderBoard(data);
	});
	
	socket.on('UI_TIMER_UPDATE', function(timerValue, turnCount) {
	...
	socket.on('time out', function(playerWhoMoved) {
		...
		if (you != playerWhoMoved) {
			...
			if (audioEnabled) timeoutAudio.play(); // play audio
		}
	});
	...
	*/
	...
	socket.on('SETUP_STORE_ID', function(id, displayName, ghost) {
		you       = id;
		yourName  = displayName;
		isGhost   = ghost; // ghost = same session open in a secondary tab
		ghostFlow = isGhost; // ghostflow = auto spectate the original session's matches(?)
	});

	
	socket.on('SETUP_ENABLE_CHAT', function(username) {
		$("#chat").remove();
		...
		$("#chatInput").on('keydown', function (e) {
			if (e.keyCode == 13) {
				...
				//if chatMessage isnt empty, send it
				if ($.trim(chatMessage)) socket.emit('CHAT_MSG', chatMessage);
			}
		});
	});	
	...
	socket.on('LOG_CHAT_MSG', function(msg, special) { log(msg, special); });

	// new: clear chat log
	socket.on('LOG_CHAT_CLEAR', function() { clearLog(); });
	
	socket.on('LOG_CONSOLE', function(data) { console.log(data); });

	//todo: implement this on server side & make a local function so client can still call audio 
	socket.on('PLAY_AUDIO', function(audioFilename) {
		if (!audioEnabled) return;
		playAudio('sfx/'+ audioFilename +'.ogg');
	});
	
	//use this to play an audio file locally
	//todo: add volume modifier arg for time-out beeps
	...
	socket.on('connect audio', function() {
		if (audioEnabled) connectAudio.play();
	});
	...
	*/
	...
	socket.on('collision', function() {
		...
		if (audioEnabled) collisionAudio.play();
	});

	socket.on('detonate', function() {
		if (audioEnabled) detonateAudio.play(); // play the audio for placing a block onto the board.
	});
...
function log(str, special) {
	if ($(".logLine").length >= 500) {
		$(".logLine").first().remove(); // don't let the DOM get out of control.
	}
...
function clearLog(){
	$('#log').empty();
	$("#log").append('<div class="logLine">Chat log cleared.</div>');
	$("#logContainer").scrollTop($('#log')[0].scrollHeight);
}

function audioButtonSVG() {
	var SVGString = '';
	SVGString += '<svg version="1.2" baseProfile="tiny" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 400 400" overflow="scroll" xml:space="preserve">';
