...
//const USE_HTTPS = true; //enable/disable https
...
var httpsOptions = {
	...
	passphrase: CREDENTIALS.sslCertPassword
};
*/
...
var HOSTNAME    = 'https://cblox.fun';
var APP_NAME    = 'cblox';
var APP_VERSION = 1.0; // todo: get this from package.json instead
function getVersion(){ return parseFloat(Math.round(APP_VERSION * 100) / 100).toFixed(2); }


// timing how long server booting up takes.
// takes longer if production bc i minify the client js which takes >10sec.
var start = new Date();
...
console.log('Running '+ APP_NAME +' v'+ getVersion() );
console.log('NODE_ENV : '+ process.env.NODE_ENV);
console.log('HOST/PORT: '+ HOSTNAME +':'+ PORT );
...
var handleDBResult = function(err, User, db) {
	...
	passport.use('twitter', new TwitterStrategy({
		...
		callbackURL    : HOSTNAME +':'+ PORT +'/login/twitter/callback'
	},
	...
	*/
	...
	io.on('connection', function(socket){
		...
		function welcome(socketID) {
			...
			socket.emit('log', '<b>Welcome to '+ APP_NAME +' v'+ getVersion() +'!</b>');
			...
			if (saveData) {
				...
				socket.emit('log', 'bug @chrstew with any and all questions');
			} else {
		...
		*/
		...
		socket.on('send chat message', function(chatMessage) {
			...
			if( chatMessage.length > 140 ) return;
			
			function isNarcissa(twitterID) {
				if (twitterID == 130360992) return true;
				return false;
			}
			...
			switch( splitMessage[0] ){
				...
				case '/newcolor':
					//if( room === 'lobby' && isNarcissa(twitterid) ){ newColor(); }
					newColor();
					break;
					
				case '/debug':
					if( isNarcissa(twitterid) ){
		...
		socket.on('attempt color reroll', function() {
			if (userData[socket.id].remainingRerolls > 0) {
				...
				newColor();
				if (userData[socket.id].remainingRerolls <= 0) socket.emit('ran out of rerolls');
			} else {
		...
		socket.on('join game', function(gameID, joinStatus) {
			if (userData[socket.id].room == 'lobby') { // if in lobby
				if (gameExists(undefined, gameID)) {   // if game exist
					if (userData[socket.id].ghost) joinStatus = 'spec';
				...
				if (gameData[gameID].gameState == 'gameover' && gameData[gameID].noRematch == false) {
					...
					if (numPlayers !== numYesRematch) {
						...
						*/
						...
						if (iceBoard) io.to(gameID).emit('log', '<span class="coldWeather">Cold weather!</span>');
	...
	function unreadyAll(gameID) {
		...
		if (refresh) io.to(gameID).emit('render board', gameData[gameID].board);
	}
	...
	*/
	...
	function performTurn(gameID, tBlock, tBlock2) {
		...
		if ((tBlock == 'pass') || (tBlock2 == 'pass')) passedTurn = true; 
		...
		if ((thisX == thatX) && (thisY == thatY)) {
			...
			if (thisType == 'reclaim') reclaim(thisType, thisX, thisY, thisOrigin);
			if (thatType == 'reclaim') reclaim(thatType, thatX, thatY, thatOrigin);
		}
		...
		if (gameData[gameID].gameState == 'inprogress') {
			...
			if (thatType == 'mine explosion' || thisType == 'mine explosion') io.to(gameID).emit('detonate'); // sfx;
	...
	function getColor(gameID, possession){
		...
		if (possession.length > 2) {
			io.to(gameID).emit('log', '<b>sorry i need to add mixing of 3 or more colors.</b>');
			return emptyColor;
		}
	}
	...
	function renderLobby(socket) {
		...
		var handleResult = function(err, leaderD) {
			...
			// no error
			if (lobbyUserData !== false) socket.emit('render lobby', lobbyD, leaderD, lobbyUserData);
		}
